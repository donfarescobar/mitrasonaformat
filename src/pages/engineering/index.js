      ...
      <>
        ...
        <div className="px-4 py-10 md:px-8 lg:px-12 xl:px-24 lg:py-12">
          ...
          {ENGINEERING.map((engineering) =>{
            const{id, img,title,description, path} = engineering;
            return(
              <div key={id} className="flex flex-col flex-start gap-3 md:gap-6">
              <div>
                <img className="object-cover h-36 lg:h-56 rounded-md w-full" 
                src={img}
                alt={`Gambar ${title}`} 
                loading="lazy"/>
              </div>
              <div className="flex flex-col gap-2.5 lg:gap-3.5">
                <h3 className="product-card-header text-xl lg:text-2xl">{title}</h3>
                <p className="engineering-page-card-description text-sm md:text-base">{description?.slice(0, 61)}...</p>
                <div>
                  <Link to = {`/engineering/${path}`}
          ...
          <section className="background-pdf my-14">
            <div className="px-4 py-10 md:px-8 lg:px-12 xl:px-24 lg:py-12">
              <div className="flex flex-row justify-between items-center gap-6 md:flex-row">
                <div className="inline-flex items-center">
                  ...
                  <div className="flex flex-col gap-1 ml-3">
                    <h3 className="pdf-header text-white text-md md:text-xl lg:text-2xl">
